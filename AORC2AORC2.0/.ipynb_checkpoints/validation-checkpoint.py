...
def get_data(year, input_source, target_source, mode):

    if mode == 'GCM':
        input = readFile(f'/scratch/jl14811/{input_source}/pr_{year}*.nc', 'pr', 1, 21, 31) * 3600
    else:
        input = readFile(f'/scratch/jl14811/{input_source}/APCP_surface_{year}*.nc', 'APCP_surface', 24, 21, 31)
    ...
    target = readFile(f'/scratch/jl14811/{target_source}/APCP_surface_{year}*.nc', 'APCP_surface', 24, 126, 186)
    target = target.reshape(-1, 4, 6, 126, 186).sum(axis=2) / 6
    if target.shape[0] == 366:
        target = np.delete(target, 59, axis=0)
...
if __name__ == "__main__":
    ...
    for year in range(args.start_year, args.end_year + 1, 1):
        ...
        input, target = get_data(year, args.input_source, args.target_source, mode=args.mode)
