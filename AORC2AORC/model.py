...
class Generator(nn.Module):
    ...
    def _initialize_layers(self):
        ...
        '''normalization layer'''
        self.input_bn = nn.BatchNorm3d(num_features=1)

        self.time_embed = nn.Embedding(20, self.filter_size)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        with record_function("input_bn"):
            nvtx.range_push("input_bn")
            x = self.input_bn(x) # I add a batchNorm layer, the original model doesn't have this
            nvtx.range_pop()
        with record_function("res1"):
            nvtx.range_push("res1")
            x1 = self.res1(x)
            x1 = CustomDropout(p=0.2,)(x1)
            nvtx.range_pop()
        with record_function("res2"):
            nvtx.range_push("res2")
        ...
        with record_function("res3"):
            nvtx.range_push("res3")
            x2 = checkpoint(self.res3,x2_stay)
            x2 = checkpoint(CustomDropout(p=0.2), x2)
            #x2 = self.res3(x2_stay)
            #x2 = CustomDropout(p=0.2, )(x2)
            nvtx.range_pop()
            
        with record_function("upsample0"):
            nvtx.range_push("upsample0")
            x2 = checkpoint(self.up0, x2)
            x2 = checkpoint(self.res4, x2)
            #x2 = self.up0(x2)
            #x2 = self.res4(x2)
            nvtx.range_pop()

        with record_function("upsample1"):
            nvtx.range_push("upsample1")
            x2 = checkpoint(self.up1, x2)
            x2 = checkpoint(self.res5, x2)
            #x2 = self.up1(x2)
            #x2 = self.res5(x2)
            x2 = CustomDropout(p=0.2, )(x2)
            nvtx.range_pop()
        ...
        with record_function("upsample2"):
            nvtx.range_push("upsample2")
            x2 = checkpoint(self.up2, x2)
            x2 = checkpoint(self.res6, x2)
            #x2 = self.up2(x2)
        ...
        with record_function("upsample3"):
            nvtx.range_push("upsample3")
            try:
                x2 = checkpoint(self.up3, x2)
                x2 = checkpoint(self.res7, x2)
                #x2 = self.up3(x2)
                #x2 = self.res7(x2)
                nvtx.range_pop()
        ...
        with record_function("res8"):
            nvtx.range_push("res8")
            x2 = checkpoint(self.res8, x2)
            #x2 = self.res8(x2)
            nvtx.range_pop()

        with record_function("res9"):
            nvtx.range_push("res9")
            try:
                x2 = checkpoint(self.res9, x2)
                #x2 = self.res9(x2)
                nvtx.range_pop()
            except torch.cuda.OutOfMemoryError as e:
                print("OOM res9")
        ...
        with record_function("output_conv"):
            nvtx.range_push("output_conv")
