...
class Generator(nn.Module):
    def __init__(self, filter_size: int = 48):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # 获取形状
        B, C, T, H, W = x2.shape  # x2 是 (B, C, 20, H, W)

        # 构造时间索引
        time_idx = torch.arange(T, device=x2.device)  # [0, 1, ..., 19]
        time_embed = self.time_embed(time_idx)  # (20, C)
    
        # reshape 成 [1, C, T, 1, 1] 广播加到输出特征上
        time_embed = time_embed.permute(1, 0).view(1, C, T, 1, 1)  # (1, C, T, 1, 1)
        x2 = x2 + time_embed
...
class Discriminator(nn.Module):
    def __init__(self, filter_size: int = 48):
        ...
        self.conv2 = ResidualBlock3D(f, f, use_layer_norm=True, stride=(1,2,2),)
        self.conv3 = ResidualBlock3D(f, f, use_layer_norm=True, stride=(2,1,1), )
        self.conv4 = ResidualBlock3D(f, f, use_layer_norm=True, stride=(2,3,3), )
        self.conv5 = ResidualBlock3D(f, f//2, use_layer_norm=True, stride=(1,5,5), )
        
        
        # LOW RESOLUTION layers
        self.conv1_1 = ResidualBlock3D(1, f//2, use_layer_norm=False, stride=(1),)
        self.conv1_2 = ResidualBlock3D(f//2, f//4, use_layer_norm=False, stride=(1),)
